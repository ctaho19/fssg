...
def test_calculate_metrics_multi_control_success():
    ...
    with patch(
        'pipelines.pl_automated_monitoring_cloudradar_controls.pipeline.refresh'
        ) as mock_refresh:
        mock_refresh.return_value = "test_token"
        
        # Mock API responses
        with patch(
            'pipelines.pl_automated_monitoring_cloudradar_controls.pipeline.OauthApi'
            ) as mock_oauth:
            ...
            def send_request_side_effect(url, request_type, request_kwargs, **kwargs):
                ...
                if (
                    search_params 
                    and search_params[0].get("resourceType") == "AWS::KMS::Key"
                ):
                    return generate_mock_api_response(_mock_kms_resources())
                elif (
                    search_params 
                    and search_params[0].get("resourceType") == "AWS::EC2::Instance"
                ):
...
def test_kms_key_rotation_control_1077224():
    ...
    with patch(
        ...
        with patch(
            ...
            mock_api_instance = Mock()
            mock_oauth.return_value = mock_api_instance
            mock_api_instance.send_request.return_value = (
...
def test_api_error_handling():
    ...
    with patch(
        ...
        with patch(
            ...
            mock_api_instance = Mock()
            mock_oauth.return_value = mock_api_instance
            mock_api_instance.send_request.side_effect = RequestException(
                "Connection error"
                )
            
            with pytest.raises(
                RuntimeError, match=r"Failed to fetch .* resources from API"
                ):
...
def test_run_function():
    ...
    with patch(
        'pipelines.pl_automated_monitoring_cloudradar_controls.pipeline.PLAutomatedMonitoringCloudradarControls'
...
def test_compliance_status_determination():
    ...
    with patch(
        ...
        with patch(
            ...
            mock_oauth.return_value = mock_api_instance
            mock_api_instance.send_request.return_value = (
                generate_mock_api_response(high_compliance_resources)
            )
