...
@transformer
def calculate_metrics(thresholds_raw: pd.DataFrame, macie_metrics: pd.DataFrame, macie_testing: pd.DataFrame, historical_stats: pd.DataFrame, context: Dict[str, Any]) -> pd.DataFrame:
    """
    ...
    for threshold in thresholds:
        ...
        if tier == "Tier 0":
            ...
            metric_value, compliant_count, total_count, non_compliant_resources = _calculate_tier0_metrics(macie_metrics)
            
        elif tier == "Tier 1":
            # Tier 1: Completeness - Calculate percentage of buckets scanned vs total CloudFronted buckets
            metric_value, compliant_count, total_count, non_compliant_resources = _calculate_tier1_metrics(macie_metrics)
...
def _calculate_tier0_metrics(macie_metrics: pd.DataFrame):
...
def _calculate_tier1_metrics(macie_metrics: pd.DataFrame):
...
def _calculate_tier2_metrics(macie_testing: pd.DataFrame, historical_stats: pd.DataFrame):
