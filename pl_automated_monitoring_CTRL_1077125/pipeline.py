...
# Control IDs and info for KMS Key Origin
KMS_KEY_ORIGIN_TIERS = {
    "CTRL-1077125": {
...
def run(
    ...
    """
    pipeline = PLAmCTRL1077125Pipeline(env)
...
class PLAmCTRL1077125Pipeline(ConfigPipeline):
    ...
    def extract_kms_key_origin(self, threshold_df: pd.DataFrame) -> pd.DataFrame:
        """Extract KMS Key Origin compliance data."""
        control_id = "CTRL-1077125"
        current_date = datetime.now()
        
        # Get config for this control
        control_config = KMS_KEY_ORIGIN_TIERS.get(control_id, {})
        ...
        tier1_metric_id = control_config.get("tier1_metric_id", "MNTR-1077125-T1")
        tier2_metric_id = control_config.get("tier2_metric_id", "MNTR-1077125-T2")
        ...
        try:
            ...
            logger.error(f"Error in extract_kms_key_origin: {e}", exc_info=True)
    ...
    def extract(self) -> DataFrame:
        ...
        logger.info("Starting extraction for CTRL-1077125 (KMS Key Origin)")
        df = super().extract()
        
        # Get threshold data from Snowflake
        threshold_df = df.get("threshold_df", pd.DataFrame())
        
        # Extract KMS Key Origin data
        df["kms_key_origin_df"] = self.extract_kms_key_origin(threshold_df)
